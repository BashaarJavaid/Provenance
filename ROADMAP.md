...
**Item 0.5 is done (Aug 21); item 1 is in progress — the repo scaffold has landed, the GCP half is pending.** Hard deadline: **August 31, 2026, 5:00 PM PDT** (All Things Agentic Hackathon); the hosted URL and credentials must stay valid through **October 1** for judging.
...
1. GCP project, ADK 2.0 project scaffold, Gemini 3.5 Pro/Flash access, Firestore provisioned, PortunusMCP added as a library dependency (per the item-0.5 resolution). Repo scaffold: `pyproject.toml`, lint/typecheck/test tooling, CI. — verify: a fresh clone installs and `pytest` runs (zero tests is fine); CI is green.
   — **in progress** (Aug 21): the repo half is done — `pyproject.toml` (package `provenance/`, runtime deps `google-adk==2.7.1` + `cryptography` only), `provenance/__init__.py`, ruff / mypy-strict / pytest config, `.github/workflows/ci.yml` running all four gates with no cloud credentials and no `pip check`, and the item-0.5 carry-forward `tests/test_portunus_surface.py` (sign/verify, ABAC compile/evaluate, plus `ATTRIBUTE_ROOTS` and `DecisionOutcome` pinned so a Portunus change breaks the build instead of surprising item 7). Verified: a fresh clone installs with the documented two-step command and `pytest` passes; dropping the `--no-deps` step turns the surface test red. **ADK is a dependency only** — no `adk create` template, no root agent, no app; that is item 3 and Phase 3. Deviation from the item text: nothing else was pulled forward, so Firestore/OpenTelemetry/Vertex clients are not declared yet — each lands with the item that first imports it.
   — **remaining**: the GCP half. `scripts/gcp_setup.sh` (idempotent; project `provenance-hackathon`, region `us-central1`) provisions the project, enables `aiplatform` + `firestore`, creates the Native-mode `(default)` database, and probes `gemini-3.5-pro` / `gemini-3.5-flash` for real access. It fails closed if no billing account is linked. It has **not been run** — `gcloud` is not installed on the dev machine; strike this item once the run output is recorded here.
