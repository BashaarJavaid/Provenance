...
**Nothing is built yet. Phase 1 is next.** Hard deadline: **August 31, 2026, 5:00 PM PDT** (All Things Agentic Hackathon); the hosted URL and credentials must stay valid through **October 1** for judging.
...
0.5. **Day-one dependency spike — before any code is written.** Three findings from auditing PortunusMCP as a dependency (see `docs/adr/ADR-004`), resolved in an hour, not discovered on Aug 25:
   - **Package naming:** the `portunusmcp` wheel ships its code as a top-level package literally named `services` (`from services.gateway import signing`). Provenance's own package must therefore be named `provenance/`, never `services/` — a local `services/` package would shadow the installed one. Update the Commands sections of `CLAUDE.md` / `AGENTS.md` / `.cursor/rules/provenance.mdc` (currently `mypy services/`) in the same commit that creates the real layout.
   - **Dependency resolution:** PortunusMCP exact-pins a heavy train (`fastapi==0.115.6`, `sqlalchemy`, `redis`, `mcp==1.28.1`, …) and requires Python `>=3.12,<3.13`. Run `pip install portunusmcp google-adk` in a clean venv as the literal first command. If it conflicts: install portunusmcp with `--no-deps` + add `cryptography` directly (safe — the modules we use, `signing`/`abac`/`decision`, are pure), or fall back to ADR-004's escape hatch and write the thin primitives as new code here.
   - **Short-lived credentials:** PortunusMCP provides static API keys and HMAC-signed requests, **not** short-lived per-agent credential minting. Decide now: build a thin minting layer on top of its signing primitives (new, scored work — preferred), or soften the "short-lived per-agent credentials" wording in `README.md` / `ARCHITECTURE.md` §2.1 before a judge asks.

   — verify: the combined install resolves (or the fallback is chosen and recorded here); `from services.gateway import signing, abac, decision` works in the venv; the credentials decision is written into this item's done note.
