...
**As built (ROADMAP items 14, 15 and 19), stages 5 and 6.** Which threshold a proposal faces is decided by one mechanical fact — whether its status differs from the version in force — and never by a judgment: 0.50 for a new belief or a re-affirmation, 0.70 for a flip. §6.3's corroboration requirement is a **set difference over source classes**: a flip commits only if its novel evidence contributes a class the accumulated set does not already carry, which is read from the same `evidence/{id}` documents stage 3 already resolved. The ways a flip can fail keep their own names — `FLIP_UNSUPPORTED` when the corroboration is what stopped it, and, when the number is, one of two depending on how far short it fell (item 19): **`INSUFFICIENT_FOR_FLIP`** if it still cleared the 0.50 new-belief door, `BELOW_THRESHOLD` if it cleared neither. Both carry `threshold = 0.70`; only the second is a statement about the agent rather than about the door, and only the second costs standing. Stage 6's "standing counter incremented" is a real write: `BELOW_THRESHOLD`, `FLIP_UNSUPPORTED`, `NO_NEW_EVIDENCE` and `RETRACTION_UNSUPPORTED` — the refusals that are statements about the proposing agent's evidence — append to its `rejection_window`, and the third inside §3.4's window writes `DEGRADED`. Infrastructure refusals do not, because standing has no automatic restoration path and an outage is not the agent's doing; nor does `INSUFFICIENT_FOR_FLIP`, whose evidence *was* verifiable and would have carried a belief of its own, nor `NOTHING_TO_RETRACT`, which is about the store's state. Reasoning in [`docs/adr/ADR-018`](./docs/adr/ADR-018-the-conflict-rule-and-the-standing-counter.md) and, for the split, [`ADR-022`](./docs/adr/ADR-022-the-refuted-and-inconclusive-paths.md).

**As built (ROADMAP item 23), the class-belief path.** §6.2's proposals run through these same six stages and add two refusals to the vocabulary rather than a second pipeline. `CLASS_BELIEF_NOT_EVIDENCE` fires immediately after stage 2 and before stage 3: it needs a store read, and item 12's rule is that the registry is read first, so it cannot live at stage 1 — which as written is well-formedness, and the frozen `Evidence` dataclass is already that. It is the fifth member of `COUNTED_REJECTIONS`, on the test that sorted the other four: citing an advisory generalization as though it were an observation is a statement about the evidence the agent brought. `INSUFFICIENT_CONSTITUENTS` is not counted, on `NOTHING_TO_RETRACT`'s test: how many entity beliefs exist is a fact about the store's population. Stage 4 is where §6.2's cap applies, over an evidence set the pipeline derived rather than one the caller passed. Stage 5 is untouched — a first class belief is not a flip, so it faces the 0.50 door like any other new belief. Reasoning in [`docs/adr/ADR-025`](./docs/adr/ADR-025-class-beliefs-and-the-advisory-cap.md).
...
**As built (ROADMAP item 23):** `derived_from` is implemented, on `BeliefVersion` beside `statement` and defaulting empty for the same reason. There is **no `class` field**: `class` is a Python keyword, so a dataclass field would have to be spelled differently from the document key on the one object whose stored form is the audit record — and it would carry exactly what `entity` and `belief_id` already do, since a CLASS belief's `entity` *is* its class name and its id is `belief_id_for()` of it. The `ADVISORY_ONLY` cap is not a field at all but two mechanisms: `recall.Recalled` keeps class beliefs in their own tuple so they cannot reach `authorizations/{id}.belief_ids` by type (item 16), and the Policy Engine refuses `CLASS_BELIEF_NOT_EVIDENCE` when a proposal cites one as evidence (§6.2, §2.2). A cap that is a field is a label; a cap that is a refusal is a guarantee. Reasoning in [`docs/adr/ADR-025`](./docs/adr/ADR-025-class-beliefs-and-the-advisory-cap.md).
...
**As built (item 21) — it is a per-domain lookup now, and both published values are 30 days.** `policy.HALF_LIFE_DAYS` is keyed by domain and read everywhere the number is used: in `contributions()`, in the `half_life_days` stored on a version, and in `expires_at`. An unpublished domain raises rather than defaulting, the posture `risk.BASE` takes, and `tests/test_policy.py` asserts every domain a registry record may write beliefs in has an entry — a fail-closed raise is only fail-closed if it cannot fire in practice. What did not happen is inventing a difference: a longer supply-chain half-life is arguable, since a contractual record ages more slowly than a live service observation, but nothing here has measured it, and changing the clock would move `SUP-042`'s live recomputed confidence and its `expires_at` on a chain items 27 and 28 attack. A future value is one line in one dict; a future value chosen today would be a number with no provenance. `docs/adr/ADR-024` records it, and answers ADR-017's and ADR-018's revisit clauses.
...
**As built (item 21).** The Supply-Chain Agent is `provenance/agents/supply_chain.py` and needed no registry record — `supply-chain-agent` has existed since item 5 with `memory_domains=("supply-chain",)` and `tool_scope=()`, and needs no tool scope because §2.1 has the Planner emit the Action. Adding it did **not** cost zero lines elsewhere, and `docs/adr/ADR-024` names every one rather than engineering around them: the graph built one domain node, session state was seeded from `company.service()` unconditionally, the Planner's instruction named config versions and an error rate directly, `recall.query_text()` called every target a service, and `half_life_domain` was one number. Each is now comprehended out of `incident.DOMAINS` or owned by the domain file, so the prediction this makes — and the thing a third domain would falsify — is that the *next* domain costs one file and one entry. **Item 22 measured what this one cost** ([`docs/generality-report.md`](./docs/generality-report.md)): 114 lines in the agent file, **zero** in a registry entry (`supply-chain-agent` has existed since item 5), zero in the gateway, the risk table, the Sweeper or the Orchestrator — and **417 touched lines, 207 of them behavioural, across nine other modules**, `policy.py` among them. §18's "zero lines changed in … the Memory Policy Engine" is therefore not true as built: the half-life became a per-domain lookup. No *decision path* moved, which is the narrower claim the evidence actually supports. The report's prediction for a third domain is **≈10 lines** outside its agent file.
...
**As built (items 10, 12, 13, 14, 15 and 19).** Item 19 added §2.2 no branch, which is the point of its entry here: a refuted verification is an ordinary `commit()` with a different status, so the pipeline decides what a refutation may write exactly as it decides everything else. What it did add is a *split of an existing refusal*, because that path exposed `BELOW_THRESHOLD` carrying two incompatible meanings. A flip that misses the 0.70 door while still clearing 0.50 is now **`INSUFFICIENT_FOR_FLIP`** — the evidence was verifiable and would have carried a belief of its own, so it is an honest agent meeting a higher door and it does **not** cost standing. A proposal below both doors is the poisoning case, keeps `BELOW_THRESHOLD`, and still does. `COUNTED_REJECTIONS` is unchanged at four reasons; the new one simply is not in it. `provenance/policy.py` runs all of §2.2: the request-time registry read (standing must be `GOOD` **and** the domain must be in `memory_domains`; an unreadable registry is a `REJECT`, not a pass), the mechanical novelty check over `(source_id, observed_at)`, `confidence()` as §4.3's noisy-OR over distinct source classes with age decay across the accumulated evidence, **both** thresholds — 0.50 for a new belief or a re-affirmation, 0.70 for a status flip — **§6.3's different-source-class rule as a set difference over the classes the version in force rests on**, then sign and append a real superseding version through `provenance/beliefs.py`. A rejected write whose cause is the agent's own evidence increments its `rejection_window`, and the third inside §3.4's window writes `DEGRADED` — §2.2 stage 6, in full. `retract()` is the second public door (§6.4): §6.4's gate replaces §6.3's, and the retraction flags the audit ledger before it appends. Nothing here is stubbed any more. The write is Firestore `create`, so a concurrent one loses rather than clobbers. Every outcome, refusals included, is signed and lands on a `belief.commit` span. Reasoning in [`docs/adr/ADR-014`](./docs/adr/ADR-014-execution-and-the-stub-policy-engine.md), [`ADR-016`](./docs/adr/ADR-016-the-versioned-belief-store.md), [`ADR-017`](./docs/adr/ADR-017-novelty-and-the-accumulated-evidence-set.md), [`ADR-018`](./docs/adr/ADR-018-the-conflict-rule-and-the-standing-counter.md) and [`ADR-019`](./docs/adr/ADR-019-retraction-and-the-audit-ledger.md).
...
**As built (ROADMAP item 24), the cap on a cold entity.** The first half of this section — "raise hypothesis priority for entities the system has never seen" — was structurally true from item 16 and had never been run by a fleet. It has now: an `error_rate` deviation on `pricing-api`, a tier-2 service with no config history and no beliefs of its own, nominates `belief-service.tier2` and hands it to the SRE agent before the first hypothesis exists, while `authorizations/{id}` cites `[]`. That is the advisory cap cold rather than warm, and it is the harder case: with an entity belief present, an excluded class belief shows up as a `belief_ids` holding exactly the other one, whereas cold the field is empty either way. The incident ends `ESCALATED` — the executor refuses an action `pricing-api` cannot receive — so nothing verifies and §7.2 permits nothing to be learned, which is what keeps that service belief-free for the next run of the same beat. What the item does **not** claim is that the generalization *caused* the hypothesis; what it can say, and item 18 could not, is that the SRE agent's own config-regression hint is conditioned on a version gap this service does not have, so it cannot have been the source. Item 32's A/B is still where the number comes from. Reasoning in [`docs/adr/ADR-026`](./docs/adr/ADR-026-incident-three-and-the-cold-entity.md).
...
**As built (item 15).** `policy.retract()` is a second public entry point beside `commit()`, sharing §2.2's pipeline: stages 2 and 3 are identical — standing, domain authority, and the same mechanical novelty check, so a belief cannot be retracted by re-citing the evidence that established it — and stages 4 and 5 swap in §6.4's gate. §6.3's different-source-class rule does **not** apply here; §6.3 hands the disproven case to §6.4 rather than governing it, and requiring a *different* class would make a sensor's own correction impossible, which is the case §6.4 exists for. The gate is two independent tests. The number is **confidence over the disproving evidence alone**, against the 0.50 door — over the accumulated set any threshold is free, since that set has already cleared one. The class test is `max(BASE_WEIGHT)` over the disproving items against `max(BASE_WEIGHT)` over the classes the version in force rests on, using the published base weights rather than decayed ones, since §6.4 compares classes and a rule that changed answer with the clock would let a belief drift out of reach. Together they say exactly what the bullet says: one `verified_system_observation` (0.60), `third_party_audit` (0.55) or `contractual_record` (0.50) can retract a belief of equal or lesser standing, while `agent_inference` (0.15) and `unverified_external_claim` (0.00) can retract nothing. `RETRACTION_UNSUPPORTED` names the class failure and counts against standing; `NOTHING_TO_RETRACT` names an absent or already-retracted belief and does not.
...
**As built (ROADMAP item 16).** `provenance/recall.py` is both reads. The exact-key half resolves `belief-{entity}` through `beliefs.current()` and involves no model at all; the index half embeds every CLASS statement and the incident's typed facts with Vertex AI `text-embedding-005` at query time, ranks by cosine, and returns at most `NOMINATE_K = 3` ids above `SIMILARITY_FLOOR = 0.55`. **No vector is stored and no index endpoint is deployed** — one bills by the hour against a corpus in the single digits, the other is an artefact that can disagree with the statement it came from (`docs/adr/ADR-020`). The index's blindness is structural rather than promised: `beliefs.class_statements()` serves **root documents**, which carry `statement` and `scope` and no status and no confidence, so it has no way to see currency even by accident. What it nominated is published beside what survived (`recall.nominated_ids`), because a span carrying only the survivors cannot tell a retracted belief being dropped from an index that found nothing. The two halves fail independently: an unreachable embedding API costs the class nominations and leaves §6.1's mechanical read untouched, while an unreadable store raises (§7.3). Class beliefs reach the prompt and the trace but **never** `authorizations/{id}.belief_ids` — §6.2 caps them as advisory, and that field is the record of what an action rested on. Until item 23 produces a class belief, the store holds none and `nominate()` returns before it spends anything; the mechanism is proven against the hand-seeded fixtures in `scripts/verify_recall.py`, the posture items 8, 14 and 15 used.
...
- **Loops are bounded by construction.** Every incident carries a retry budget: one bounded re-plan after a `REFUTED` verification, then mandatory escalation. No agent owns its own iteration count — the control loop does, in code.
  **As built (item 20):** `resolve` is a routing node and the re-plan is a real edge, `(resolve, {"REPLAN": planner_agent, "DONE": halt})` — the same shape §7.1's malformed re-plan has had since item 9, and the reason [`ADR-007`](./docs/adr/ADR-007-adk-orchestration-park-resume.md) chose a declarative graph in the first place. The count is `scratch.refuted_attempts` against `incident.REFUTED_RETRY_BUDGET` (1), and it is **separate from the malformed count** because this bullet and the one above describe different failures: a Planner that fumbled a field once must not thereby lose the fleet's only chance to fix the service. One incident may spend both, which is three Planner calls and still bounded. Only `REFUTED` is retried — §7.2 gives `INCONCLUSIVE` its own row. `attempt` on the `verification.outcome` span finally exceeds 1, and "no third attempt" is checked by counting spans rather than by a new attribute. Reasoning in [`docs/adr/ADR-023`](./docs/adr/ADR-023-the-bounded-retry.md).
...
**As built (items 10 and 19).** All three rows are live and the two that are not `CONFIRMED` are exercised by fault injection rather than asserted. `provenance/incident.py`'s `_resolve` reads §7.2's table out of a two-entry dict, `_LEARNS_FROM`, mapping the outcomes memory learns from to the status each teaches — `INCONCLUSIVE` is **absent from that dict** rather than mapped to nothing, so "no partial credit" is the shape of the data and not a branch that can be softened. The negative belief's status is `ROLLBACK_INEFFECTIVE`: §7.2's sentence is a claim about the *remediation*, and a rollback that failed to help does not show the config was innocent. It goes through the ordinary `policy.commit()` door and **not** through §6.4's retraction, for the same reason — the belief a confirmed run wrote is not thereby wrong. One consequence is worth knowing and is correct: on a service already carrying the confirmed status the negative belief is a status *flip*, so it faces 0.70 and §6.3's different-source-class rule and one fresh `verified_system_observation` cannot carry it. The engine decides that, not the control loop. Reasoning in [`docs/adr/ADR-022`](./docs/adr/ADR-022-the-refuted-and-inconclusive-paths.md).

**As built (item 20) — the `REFUTED` row's first clause.** The bounded retry is live, so a first refutation is no longer terminal: the Planner re-plans once with the refutation as input, and a second refutation escalates with the incident still open. The retry's commit is a **re-affirmation** — same status, same source class — so it faces the 0.50 door rather than 0.70 and supersedes at the same 0.60. What accumulates is evidence, not certainty (item 18's lesson, inside one incident this time). Making that reachable took one real fix: each attempt now stamps its evidence with its **own** observation time, because two attempts sharing the incident's frozen `now` are one observation cited twice, and §2.2 would refuse the second `NO_NEW_EVIDENCE` — a *counted* rejection, so an honest agent would lose standing for reporting that its own remediation failed. Reasoning in [`docs/adr/ADR-023`](./docs/adr/ADR-023-the-bounded-retry.md).
