...
7. **Hold/resume** — a held action parks the incident; the approval card (§8) lands in the store operations manager's queue; the incident resumes on approve/deny.
...
**As built (item 30) — stage 7, and a second public door.** `provenance/approvals.py` is the queue: a `HOLD` writes `approvals/{id}` (id content-addressed from the held decision's signature) carrying the raw proposal, the subject, the held signature, the recalled `entity_ids`, the routed domain and agent, the three trigger facts and the park's `trace_id`. `gateway.resolve()` is the second public coroutine beside `authorize()` — the first stage-7 answer — and it re-validates the stored proposal, re-reads the registry and recomputes §4.2 rather than reading any of them back, because `_signing_key()` is per-process and a park outlives its process. New vocabulary: `AuthStage` gains `human`; `DecisionReason` gains `HUMAN_APPROVED` and `HUMAN_DENIED`; `audit.Authorization` gains `approver`. `incident.resume()` runs the remainder of the loop. `approvals.APPROVER_MAX_LENGTH = 64`. Nothing expires: §7.3's row is the whole rule. Reasoning in [`docs/adr/ADR-032`](./docs/adr/ADR-032-the-approval-queue.md), including why ADK 2.7.1's absent Task API changed the shape rather than the guarantee.
...
6. **Outcome** — COMMIT (new version, supersession link, signature), REJECT (logged, standing counter incremented), RETRACT (§6.4), or EXPIRE (§6.5, item 29 — a downgrade the clock asked for and no agent proposed). Every outcome is signed and audited.
...
Long-running async process: on belief expiry, re-verifies or downgrades to `UNKNOWN(stale)` (§6.5). One of the two async behaviours the track's runtime requirement asks for (the other is wake-on-event incident handling plus park/resume on human approval — §2.1 stage 7, built in item 30).

**As built (item 29).** `provenance/sweeper.py`: `sweep()` is one tick and `run_forever()` the loop, started as an `asyncio` task by `app.py`'s lifespan and cancelled on shutdown. `SWEEP_INTERVAL_SECONDS = 300`. It decides nothing — the eligibility rule is `policy.expire()`'s, and this module finds candidates and calls the door. The walk is `beliefs.belief_ids()` (root documents, so it cannot see currency — ADR-005) plus `current()` per belief; a belief that cannot be read or written is skipped and swept again next tick. **It runs only while a Cloud Run instance is warm**: the service is at `--min-instances=0`, so "runs continuously" is true of the process, not of the calendar. There is no `POST /sweep` — `sweeper.sweep()` is called directly by scripts and by any demo beat. **Only §6.5's no-branch is built**; [`ADR-031`](./docs/adr/ADR-031-the-staleness-sweeper.md) records why the yes-branch has no source in this system.
...
- **A status flip additionally requires at least one evidence item of a `source_class` different from the class that established the current status, carrying non-zero weight.** A single sensor cannot both set and clear an alarm — and a class that cannot move the number (§4.3) cannot break the tie either.
...
**As built (items 14 and 19).** "The class that established the current status" is the set of `source_class` values the version in force rests on — its accumulated evidence, already resolved by §2.2 stage 3 — and the rule is a set difference against it: a flip commits only if its novel items contribute a class that set does not carry. Because evidence accumulates this is the **stricter** of the two available readings, which is the right side to err on for a rule whose purpose is that one sensor cannot set and clear its own alarm. **Item 28 corrects the difference itself:** the novel side is filtered to classes whose `BASE_WEIGHT` is above 0.00, because a flip is measured over the *accumulated* set and a zero-weight item moves that number by nothing — so an unfiltered difference let `unverified_external_claim` overturn any belief already past `FLIP_THRESHOLD`, which is the state `SUP-042` was in. `known` is deliberately not filtered. No model is consulted anywhere in it. Which door stopped a flip, and how badly it missed, is §2.2's as-built above; the reasoning is [`ADR-018`](./docs/adr/ADR-018-the-conflict-rule-and-the-standing-counter.md) and, for item 28's correction, [`ADR-030`](./docs/adr/ADR-030-the-poisoning-arc-and-standing.md).
...
**As built (item 29) — the yes-branch is not built, and that is a finding rather than an omission.** Nothing in this system is a re-verification source: §5.8's Verification Agent judges one predicate declared *before* an execution, and a sweep has neither. So `policy.expire()` is the no-branch, and it is a **third public door** beside `commit()` and `retract()` — a sweep has no proposal and no proposing agent, so §2.2's stages 2–5 have nothing to act on. It re-reads the version in force itself (the walk and the write are two moments) and appends an `UNKNOWN` version when `expires_at` has passed, refusing `NOT_DUE` otherwise and on anything already `UNKNOWN` or `RETRACTED` — without which a warm instance would append a version every tick forever. The downgrade **carries the predecessor's `expires_at`, `half_life_days`, `threshold` and evidence set forward unchanged** and stores §4.3 **recomputed as of the sweep**; `authority` reads `staleness-sweeper@v1 (§6.5)` while `committed_by` stays `memory-policy-engine`. `UNKNOWN` carries no reason field: the Sweeper is its only writer and §6.5 gives it one reason, so an `UNKNOWN` version is stale by construction. Two new `CommitReason` words, `EXPIRED` and `NOT_DUE`, are the only two in that vocabulary that are not statements about an agent's evidence, so **neither is in `COUNTED_REJECTIONS`**. Reasoning in [`ADR-031`](./docs/adr/ADR-031-the-staleness-sweeper.md).
...
| `provenance.belief.commit` | Memory Policy Engine (§2.2) | `belief.{id,version,scope,domain,entity,status,confidence,threshold,supersedes}`, `evidence.{ids,source_classes,novel_count}`, and `decision.{outcome,reason,signature}`. `decision.outcome` is COMMIT / REJECT / RETRACT / **EXPIRE** |
...
| `provenance.reasoning.chain` | any **[LLM]** component | `agent.{id,version}`, `reasoning.{model,step,hypotheses_considered,selected_hypothesis,input_tokens,output_tokens,model_calls}`, `recall.{belief_ids,nominated_ids}` |
...
**Added in item 30 — `human`, and no fifth shape.** §2.1 stage 7's answer is the same stage-6 decision the other five stages produce — signed, reported, with the §4.2 components riding along — so it took an `AuthStage` word and a pair of `DecisionReason` words rather than a shape, under item 2's standing condition. The attribute set is byte-identical to a risk-stage approval's, which `tests/test_telemetry_schema.py` asserts by comparison rather than by a copied literal. `_ERROR_OUTCOMES` needed no change: it keys off the outcome, so a human denial is already `DENY` and already an error, while an approval is not. **The approver is not a span attribute** — it is on the ledger row, and this section's vocabulary is closed. A resumed leg opens a **new root span reusing the parked `incident_id`**, not the parked trace: the buffer §8.2 reads is in-process, so a leg attached to a trace whose process is gone would render with no parent. Reasoning in [`ADR-032`](./docs/adr/ADR-032-the-approval-queue.md) §§8-9.

**Added in item 29 — `EXPIRE`, and no fifth shape.** §6.5's downgrade is the same stage 6 as the other three outcomes — a status written, signed and reported — so it took a word rather than a shape, added under item 2's standing condition (module, this section and `tests/test_telemetry_schema.py` in one commit). No new attribute key. Reusing `COMMIT` was the alternative and would have made the ledger and the trace UI say the organization *committed* a belief at the moment it stopped believing it. It is deliberately **not** an error outcome: a belief reaching its own expiry date is the system working. The span's `agent.id` is `staleness-sweeper` and its `agent.standing` is `GOOD` — the Sweeper holds no registry record and has no standing that can move, but the two must not look alike to a reader ([`ADR-031`](./docs/adr/ADR-031-the-staleness-sweeper.md) §7).
...
**Amended in item 32 — `reasoning.model_calls`, the first new attribute key since item 16.** Requests to the model inside one step, incremented in `provenance/agents/_reasoning.py`'s `after_model_callback` **before** the `usage_metadata` check, so a response that reports no tokens still counts as the request it was. It is not the same as counting spans: §7.1's re-plan and any transport retry both land inside one span. It exists because item 32's `verify:` line asks for "tool calls" and this fleet has none — no agent is built with `tools=` — so the honest measure of what a step spent is requests ([`ADR-034`](./docs/adr/ADR-034-the-counterfactual-ab.md) reason 1). Still five shapes.
...
**As built (ROADMAP item 28) — the registry panel.** `GET /registry` returns every agent in `registry.AGENTS` as `id`, `version`, `standing` and `rejection_window`, read through `registry.get_agent()` at request time and cached nowhere — §1.1 property 4 is the point of the surface, since what it exists to show is a standing that changed a second ago. `public_key` is not served. `RegistryError → 503`, for §7.3's reason on the read side: "the registry was unreadable" and "every agent is in good standing" must not look alike, and an all-GOOD panel mid-outage is the one wrong answer this surface can give. The panel polls at **5s** — between the fleet view's 1s and the inspector's 15s — so a DEGRADED transition lands inside one narration beat while four documents every five seconds stays inside Firestore's free daily read tier. The `rejection_window` renders only where it is non-empty, as `HH:MM reason`; §3.4's window is 24 hours wide by construction, so the date cannot distinguish two entries. This is the second surface reading Firestore rather than the stream, for ADR-021's reason applied to standing: the span buffer is in-process and bounded, so a counter derived from it can disagree with the registry it claims to show. Reasoning in [`ADR-030`](./docs/adr/ADR-030-the-poisoning-arc-and-standing.md).
...
**As built (item 32) — the sixth surface, and the only one that is not live.** The counterfactual panel reads `GET /counterfactual`, which serves a **committed file** (`provenance/web/counterfactual.json`) rather than running anything: the A/B is twelve real incidents, so a route that re-ran them per request would spend the credit ceiling and answer a different number every time. The raw per-run evidence lives in `docs/counterfactual/`; `scripts/verify_counterfactual.py` with no arguments re-derives the table from it and refuses if the served JSON or the report's markdown has drifted from it — [`ADR-021`](./docs/adr/ADR-021-the-belief-inspector.md)'s "a breakdown that can disagree with the number it explains is worse than none", applied to a measurement. Two of §8.2's four named metrics were not what the wording assumed: **there are no tool calls** to count in this fleet (no agent is built with `tools=`), and no ordering of hypotheses is stored anywhere, so the panel shows **model calls** and a **model-asserted** hypothesis count, both labelled. The numbers and their method are in [`docs/counterfactual-report.md`](./docs/counterfactual-report.md); the decisions are in [`ADR-034`](./docs/adr/ADR-034-the-counterfactual-ab.md). Panel is fetched once and never polled — a committed file has nothing live to miss. One correction to §8 above: three of the four metrics do come off the one stream (model calls, tokens, hypothesis count), but **wall-clock does not** — it is measured in the runner around `run_incident()`, because a Cloud Trace read-back lags by up to two minutes and the honest number is the one a person waited through, recall included.
...
| Standing | Earned and lost, never quietly restored | Three rejected writes in the window → DEGRADED; fourth proposal (low-risk) requires human approval; no automatic restoration path exists. **Built (item 28)**: live in `scripts/verify_poisoning_arc.py` — the counter driven to DEGRADED by the poisoning above, the DEGRADED agent's next *memory* write refused `STANDING_NOT_GOOD` without incrementing the counter it read, and an ordinary `ROLLBACK_CONFIG` held `HOLD/registry/STANDING_DEGRADED` at 1+1+0+0=2. The two halves run on **two agents** and the script says so: no registered agent both holds a memory domain and holds a tool scope, so the gateway would deny the poisoner `TOOL_SCOPE` before standing was ever evaluated (ADR-030 §2) |
| Approval queue | Held actions park durably; nothing auto-approves | Park an incident, restart the process, resume on approve — assert `execute → verify → learn` completes and the denial is signed into the ledger; assert an unresolved park never self-resolves — **built (item 30)**: `tests/test_approvals.py` offline (including the absence, asserted as one: the module contains no clock), `scripts/verify_approval_queue.py` live, whose `--park-only` / `--resume` pair runs the two halves in two processes so "survives process restarts" is checked rather than asserted |
| Sweeper | Expiry is consumed | Expire a belief with no re-verification source; assert it is `UNKNOWN(stale)`, excluded from recall, never deleted — **built (item 29)**: `tests/test_sweeper.py` offline, `scripts/verify_sweeper.py` against real Firestore and a real embedder, which asserts the swept belief is still *nominated* before asserting the store drops it, and that a second sweep appends nothing |
...
| Injection arc | The gateway holds when filters leak | End-to-end: the §10-spec payload passes Model Armor and the sanitizer, the dangerous action is proposed, and the gateway holds it at score 11. **Built (item 27)**: `tests/test_incident.py` offline with both filters stipulated to leak — asserting the components *and* the band separately, since they fail to different mutations — and live in `scripts/verify_injection_arc.py`, one incident carrying the payload through item 21's own `check_result()`. What it claims is this row and not causation: `DISABLE_COMPLIANCE_CHECKS` is the only supplier-scoped tool, so item 21 reaches 11 on the identical trigger with no payload, and the two scripts run as a pair are the honest form of that ([`ADR-029`](./docs/adr/ADR-029-the-injection-arc.md)) |
| Poisoning arc | Arithmetic defense | End-to-end: unverifiable "cleared" claim → confidence unmoved → rejected → three attempts → DEGRADED on the registry panel. **Built (item 28)**: `tests/test_policy.py` offline; live in `scripts/verify_poisoning_arc.py`, against the real seeded `SUP-042`. Building it found the row's own premise to be false as implemented — "confidence unmoved" is not a defence when the flip is measured over the *accumulated* set, so the claim cleared 0.70 on the strength of the belief it contradicted and **committed**. What refuses it is §6.3 with item 28's weight filter, under `FLIP_UNSUPPORTED` rather than `BELOW_THRESHOLD`; the script asserts the confidence was *above* the door, without which the arithmetic could be doing the work and the class rule would never be tested ([`ADR-030`](./docs/adr/ADR-030-the-poisoning-arc-and-standing.md)) |
...
| Counterfactual | The value of memory is measured, not asserted | Run incident #2 under one variable — `run_incident(memory=...)`, which skips recall and changes nothing else — three trials per arm, interleaved, and report medians of wall-clock, model calls, tokens and the (model-asserted) hypothesis count. **Built (item 32)**: `scripts/verify_counterfactual.py --record` live; its default mode re-derives the table from the committed per-run artifacts and refuses if the served JSON or the report's markdown disagrees, which is the item's `verify:` line and runs in CI with no credentials. Both arms must end at belief v2 — an arm at v1 would mean the belief was never there, and one with no belief would mean the commit had been disabled too, either way making the delta unattributable |
