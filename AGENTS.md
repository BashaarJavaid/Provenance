...
- `PROVENANCE_PLANNER_KEY="$(cat ~/planner.pem)" GOOGLE_CLOUD_PROJECT=provenance-hackathon GOOGLE_GENAI_USE_VERTEXAI=1 GOOGLE_CLOUD_LOCATION=global .venv/bin/python scripts/verify_incident_one.py` — the item-9 `verify:` check: injects the fault, wakes the fleet against live Firestore and live Gemini, asserts exactly one typed `ROLLBACK_CONFIG(inventory-api)` scoring `1+1+0+0 = 2` and auto-approved, then reads the incident/reasoning/decision spans back from Cloud Trace. **Writes live state** — the injection sits in a `try/finally` that clears it on any exit path, and the script refuses to run if the switch is already on. Set `PROVENANCE_SERVICE_URL` + `PROVENANCE_TRIGGER_TOKEN` to also check the deployed `POST /trigger` (403 without the header, 200 with). Costs three `gemini-2.5-pro` calls, ~60s. Needs credentials, so it is not in CI
...
**Phases 1 and 2 are complete and Phase 3 is under way — items 1–9 are done (Aug 21); item 10 (execution + Verification Agent on the `CONFIRMED` path + a stub belief commit) is next.** The repo has `pyproject.toml` (package `provenance/`; runtime deps are `google-adk==2.7.1`, `cryptography`, `opentelemetry-exporter-gcp-trace`, `fastapi`, `uvicorn` and `google-cloud-firestore`), ruff / mypy-strict / pytest config, GitHub Actions CI that deliberately does not gate on `pip check`, and `tests/test_portunus_surface.py` guarding the consumed Portunus surface. ADK is now used, not just pinned: `provenance/agents/` and `provenance/incident.py` arrived in item 9. On the cloud side, `provenance-hackathon` exists with `aiplatform` + `firestore` + `cloudtrace` + `run` + `cloudbuild` + `artifactregistry` enabled and a Native-mode Firestore database in `us-central1`. **Gemini 3.5 Pro is not served to this project** (probed, 404 on every variant and endpoint), so the four reasoning roles run on `gemini-2.5-pro` and verification on `gemini-3.5-flash`; note that Gemini 3.x lives on the **`global`** endpoint, not a regional one.
