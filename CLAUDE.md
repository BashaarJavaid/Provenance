...
**Phase 1 is next — nothing has been built yet.** The repo currently contains only the documentation suite (this file, `README.md`, `ARCHITECTURE.md`, `THREAT_MODEL.md`, `ROADMAP.md`, `docs/adr/`) and the original spec. See `ROADMAP.md` for the full build order; update this section as phases complete, recording what shipped and any deviations, the way each completed item gets a `— **done**:` note in the roadmap.
