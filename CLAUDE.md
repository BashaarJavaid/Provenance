...
**Registry state note:** `remediation-planner` is at **`v2`** — item 7 ran `seed_registry.py --rotate` once, because item 5 printed the original private half exactly once and it no longer existed. Standing and `rejection_window` survived the rotation; only the key and version moved. Anything minting a credential now uses **v2**, and a proposal's `proposed_by` must read `remediation-planner@v2` or the gateway denies at `stage="identity"` — that is the version-binding check working.
