...
- `/health`, never `/healthz` (Cloud Run swallows the latter). Don't rename an `<h2>` in `provenance/web/index.html`. `POST /trigger` and `POST /approvals/{id}` are token-guarded by the **same** `PROVENANCE_TRIGGER_TOKEN` and **fail closed when it is unset** — don't remove either guard, and don't add a second secret. `GET /trace`, `GET /belief/{entity}`, `GET /registry`, `GET /approvals` and `GET /counterfactual` are unauthenticated on purpose. **`PROVENANCE_TRIGGER_TOKEN` is published in `README.md`'s "Run the demo" on purpose** (item 36): a judge with only the URL has to be able to complete the approval flow, and `--max-instances=1` plus a ~60s serial incident bounds the exposure to a queue somebody else can occupy. Don't remove it as a leak; rotate it after Oct 1.
...
- `scripts/reaffirm_demo_beliefs.py` — the decay-clock runbook, not a build item; **no arguments reads only** and exits non-zero while any clock is short, `--commit` appends one superseding version to each of the five. No model calls, nothing to restore.
...
- **A trigger on `inventory-api` at clean baseline still executes, and it consumes incident #1's fixture.** The fault switch is not what makes the arc run: `error_rate_spike` is read by **nothing** under `provenance/` (only `rollback_fails` and `verification_ambiguous` have consumers), and `sre_infra.py` builds its context from the in-code fixture, where `v42` sits above a known-good `v41` permanently. So §5.4's config-regression hint fires fault or no fault. Verified live on Aug 27 with the *old* form defaults: the incident diagnosed, scored 2, **executed** `v42 → v41`, verified `CONFIRMED`, and committed `belief-inventory-api` v1 — destroying the "cold case, no relevant memory yet" premise the beat depends on. The trigger strip now defaults to the supply-chain incident so an idle click cannot reach this (item 36). To restore, use `verify_incident_one.restore()`: it is the only teardown that clears all three — service fields, the belief chain with its evidence, and the target's ledger rows. **`seed_firestore.py --reset` does not delete the belief.**
...
- **`GET /` sends `Cache-Control: no-store` since Aug 27, and it is deployed.** The original trap (the shell HTML was not cache-busted, so a browser that loaded the page before a redeploy ran the *old* inline JavaScript against the new routes; cost half an hour during item 31) is fixed in `app.py`, pinned by `test_the_shell_is_not_cached_between_redeploys`, and live on `provenance-00025-nzt`. Hard-reloading between takes is no longer needed. Check it with a **`GET`**, not `curl -I` — the route is GET-only and answers a `HEAD` with 405.
...
- **Before 2026-09-22, re-affirm `SUP-042`.** Its v2 expires **2026-09-22** and `belief-service.tier2` expires **2026-09-24**, both inside the Oct 1 judging window — so from those dates item 29's Sweeper downgrades them to `UNKNOWN` on any warm instance, destroying the closing shot and the state items 27 and 28 both proved byte-identical. This is the Sweeper being correct, not a bug, and there is deliberately **no skip list** (`ADR-031`, the live finding). The fix is the mechanism the design already has and it now has a script: **`scripts/reaffirm_demo_beliefs.py`** commits one fresh corroborating evidence item per belief through `policy.commit()`, superseding rather than rewriting. Three things about it. **Five beliefs, not one** — the class belief's cap is derived from `checkout-api`, `orders-api` and `search-api`, all expiring 09-24, so they go first and the class is re-read after them. **Run it in September, not now**: `HALF_LIFE_DAYS` is 30, so an August run buys a clock that still expires before judging; the window is ~**Sept 8–15** and `SAFE_UNTIL = 2026-10-08` is the assertion that enforces it. **Its no-argument form is the safe one** — reads only, prints every clock, exits non-zero while any is short. `seed_belief.py` has no `--reset` and must not grow one.
