...
- `PROVENANCE_PLANNER_KEY="$(cat ~/planner.pem)" GOOGLE_CLOUD_PROJECT=provenance-hackathon GOOGLE_GENAI_USE_VERTEXAI=1 GOOGLE_CLOUD_LOCATION=global .venv/bin/python scripts/verify_incident_one.py` — the item-9 **and item-10** `verify:` check, one script because item 10 is incident #1 finishing: injects the fault, wakes the fleet against live Firestore and live Gemini, asserts exactly one typed `ROLLBACK_CONFIG(inventory-api)` scoring `1+1+0+0 = 2` and auto-approved, that the rollback executed (v42 → v41, error rate dropped), that verification returned `CONFIRMED` against the pre-declared predicate, and that one belief committed at exactly `0.60` — then reads the incident/reasoning/decision/verification/belief spans back from Cloud Trace, asserting the verification span's `predicate_id` equals the one the incident span carried *before* execution. **Writes live state** — the injection and the rollback sit in a `try/finally` that restores `error_rate`, `healthy`, `current_config_version` and deletes the belief on any exit path; the script refuses to run if the fault switch is already on **or** if `beliefs/belief-inventory-api-1` already exists. Set `PROVENANCE_SERVICE_URL` + `PROVENANCE_TRIGGER_TOKEN` to also check the deployed `POST /trigger` (403 without the header, 200 with). Costs three `gemini-2.5-pro` calls plus one `gemini-3.5-flash`, ~60s. Needs credentials, so it is not in CI
- CI (`.github/workflows/ci.yml`) runs lint, format, type-check, and tests on push/PR — no cloud credentials

Note that `./scripts/deploy.sh` now **requires** `PROVENANCE_PLANNER_KEY` and `PROVENANCE_TRIGGER_TOKEN` and refuses to run without them; it passes the whole environment as a `--env-vars-file` because a PEM is multi-line, and checks that the deployed `/trigger` answers 403 unauthenticated.
...
**Phases 1 and 2 are complete and Phase 3 is under way — items 1–10 are done (Aug 22); item 11 (the trace UI: live fleet view + gateway ledger reading the one OpenTelemetry stream) is next.** The repo has `pyproject.toml` (package `provenance/`; runtime deps are `google-adk==2.7.1`, `cryptography`, `opentelemetry-exporter-gcp-trace`, `fastapi`, `uvicorn` and `google-cloud-firestore`), ruff / mypy-strict / pytest config, GitHub Actions CI that deliberately does not gate on `pip check`, and `tests/test_portunus_surface.py` guarding the consumed Portunus surface. ADK is now used, not just pinned: `provenance/agents/` and `provenance/incident.py` arrived in item 9, and item 10 appended two nodes and a fourth agent to the same graph. On the cloud side, `provenance-hackathon` exists with `aiplatform` + `firestore` + `cloudtrace` + `run` + `cloudbuild` + `artifactregistry` enabled and a Native-mode Firestore database in `us-central1`. **Gemini 3.5 Pro is not served to this project** (probed, 404 on every variant and endpoint), so the four reasoning roles run on `gemini-2.5-pro` and verification on `gemini-3.5-flash`; note that Gemini 3.x lives on the **`global`** endpoint, not a regional one.
